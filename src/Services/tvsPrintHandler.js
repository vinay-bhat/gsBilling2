...
const sumTotalDiscount = (arr) =>
  arr.reduce((sum, { item_price, item_qty }) => sum + item_price * item_qty, 0);
const sumBasicDiscount = (arr) =>
  arr.reduce((sum, { item_basic, item_qty }) => sum + item_basic * item_qty, 0);
...
export const onCounterBillGenerate = async (custInfo) => {
  const { date, time } = getCurrentDatTime();
  ...
  if (Object.keys(groupedItemsByToken).length === 0) {
    TVSBillingModule.onCounterBillGenerate(
      outletDetails?.branch_title,
      outletDetails?.org_name,
      outletDetails?.gstin_no,
      outletDetails?.address1,
    ...
    TVSBillingModule.onCounterBillGenerateWithToken(
      outletDetails?.branch_title,
      outletDetails?.org_name,
      outletDetails?.gstin_no,
      outletDetails?.address1,
